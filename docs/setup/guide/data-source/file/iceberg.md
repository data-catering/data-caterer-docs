...
#### Join With Another Iceberg Table

Now that we have generated some accounts, let's also try to generate a set of transactions for those accounts in Iceberg
format as well. The transactions could be in any other format, but to keep this simple, we will continue using Iceberg.
...
=== "Scala"
    ...
    ```scala
    val transactionTask = iceberg("customer_transactions", "/opt/app/data/customer/iceberg", "account.transactions")
      .schema(
        ...
      )
      .count(count.recordsPerColumn(5, "account_id", "full_name"))
