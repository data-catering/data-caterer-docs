...
=== "Java"
    ...
    ```java
    var accountTask = delta("customer_accounts", "/opt/app/data/customer/delta")
            .schema(
...
#### Join With Another Delta Lake Table

Now that we have generated some accounts, let's also try to generate a set of transactions for those accounts in Delta Lake
format as well. The transactions could be in any other format, but to keep this simple, we will continue using Delta Lake.
...
=== "Java"
    ...
    ```java
    var transactionTask = delta("customer_transactions", "/opt/app/data/customer/delta-transactions")
            .schema(
                    field().name("account_id"),
...
=== "Scala"
    ...
    ```scala
    val transactionTask = delta("customer_transactions", "/opt/app/data/customer/delta-transactions")
      .schema(
        field.name("account_id"),
        field.name("full_name"),
...
=== "Scala"
    ...
    ```scala
    val transactionTask = delta("customer_transactions", "/opt/app/data/customer/-transactions")
      .schema(
        ...
      )
      .count(count.recordsPerColumn(5, "account_id", "full_name"))
